...
sealed interface EditorAction {
    ...
    data class TransformItems(val ids: List<String>, val matrix: FloatArray) : EditorAction {
        ...
        // Phase 10.1 — per-kind transform logic lives in [ItemTransformer]
        // so align / distribute can build per-item matrices through the
        // exact same payload re-encode path.
        private fun transformItem(item: NoteItem, m: FloatArray): NoteItem =
            ItemTransformer.transform(item, m)
    }
