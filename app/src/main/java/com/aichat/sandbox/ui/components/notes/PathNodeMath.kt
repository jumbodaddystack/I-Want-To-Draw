 ...
 *
 * Every function returns a fresh payload (payloads are immutable); the
 * node-edit overlay previews the result live and the ViewModel commits one
 * `CompositeEdit` per gesture. JVM-pure so the geometry is unit-testable.
 */
object PathNodeMath {
    ...
    /**
     * Closest point on the path to (`x`, `y`): coarse uniform-t scan per
     * segment, then a local refinement pass around the best sample. Null
     * for degenerate (< 2 anchor) paths.
     */
    fun nearestOnPath(payload: PathCodec.PathPayload, x: Float, y: Float): Nearest? {
        ...
        for (i in 0 until segs) {
            ...
            for (step in 0..COARSE_STEPS) {
                ...
                if (best == null || d < best!!.distance) {
                    best = Nearest(i, t, px, py, d)
                }
            }
        }
        val coarse = best ?: return null
        // Refine ± one coarse step at fine resolution.
        val s = PathCodec.segment(payload, coarse.segment)
        ...
        for (step in 0..FINE_STEPS) {
            ...
            if (d < refined.distance) {
                refined = Nearest(coarse.segment, t, px, py, d)
            }
        }
        return refined
    }
    ...
    /**
     * Insert an anchor at parameter [t] of segment [segment] via a de
     * Casteljau split — the curve's geometry is preserved exactly. The new
     * anchor lands as SMOOTH (its handles came off one curve, so they're
     * collinear by construction).
     */
    fun insertAnchor(payload: PathCodec.PathPayload, segment: Int, t: Float): PathCodec.PathPayload {
        val n = payload.anchors.size
        require(segment in 0 until payload.segmentCount) { "segment $segment out of range" }
        val s = PathCodec.segment(payload, segment)
        // de Casteljau at t.
    ...
    fun deleteAnchor(payload: PathCodec.PathPayload, index: Int): PathCodec.PathPayload? {
    ...
    /**
     * Corner ⇄ smooth toggle. Corner → smooth aligns the handles into a
     * single tangent (averaged from whatever handles exist, falling back to
     * the neighbour chord) with one-third-chord lengths when a side had no
     * handle. Smooth / symmetric → corner keeps the handles as-is — corner
     * just means the sides move independently from here on.
     */
    fun toggleType(payload: PathCodec.PathPayload, index: Int): PathCodec.PathPayload {
        val anchors = payload.anchors.toMutableList()
        val a = anchors[index]
        anchors[index] = if (a.type == PathCodec.TYPE_CORNER) {
            val (tx, ty) = smoothTangent(payload, index)
            if (tx == 0f && ty == 0f) {
                a.copy(type = PathCodec.TYPE_SMOOTH)
            } else {
                val inLen = handleLengthOrDefault(payload, index, incoming = true)
                val outLen = handleLengthOrDefault(payload, index, incoming = false)
                a.copy(
    ...
    /**
     * Retarget one handle of anchor [index] to the absolute point (`x`, `y`).
     * SYMMETRIC mirrors direction + length onto the other side; SMOOTH
     * mirrors direction but keeps the other side's length; CORNER moves the
     * dragged side only.
     */
    fun moveHandle(
    ...
    private fun smoothTangent(payload: PathCodec.PathPayload, index: Int): Pair<Float, Float> {
        val a = payload.anchors[index]
        var tx = a.outDx - a.inDx
        var ty = a.outDy - a.inDy
        if (tx == 0f && ty == 0f) {
            val prev = neighbour(payload, index, -1)
            val next = neighbour(payload, index, +1)
    ...
    private fun handleLengthOrDefault(
        ...
        val n = neighbour(payload, index, if (incoming) -1 else +1)
    ...
    private fun neighbour(payload: PathCodec.PathPayload, index: Int, delta: Int): PathCodec.Anchor? {
        val n = payload.anchors.size
        val i = index + delta
        return when {
            i in 0 until n -> payload.anchors[i]
            payload.closed -> payload.anchors[(i + n) % n]
            else -> null
        }
    }
