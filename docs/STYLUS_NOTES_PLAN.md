...
- [~] **3.4** Sketch composer attachment sheet + verification ([details](./STYLUS_NOTES_PHASE_3.md#sub-phase-34--sketch-composer-attachment-sheet--phase-3-verification)) — code landed (sketch button in composer, `SketchAttachmentSheet` reusing `DrawingSurface` in sketchMode, PNG rasterized via `NoteRasterizer` and attached to outgoing message); 10-item Phase 3 device matrix still pending on real S25 Ultra hardware
