...
- **Current phase:** Phase 9 — sub-phases 9.1 / 9.2 / 9.3 / 9.4 code-complete. Notebooks have their own table (`notebooks`, MIGRATION_11_12) and tab; `NotebooksListScreen` renders a 2-col cover grid (`NotebookCard.noteId` resolves the underlying note for navigation) and `NewNotebookSheet` picks page size / style / cover colour. Pages live as uniform `NoteFrame` rows on the underlying note; the editor branches to a `PageThumbnailRail` + `addNotebookPage` flow when `Note.notebookId` is non-null. Cross-note OCR search lives on a standalone `notes_ocr_fts` virtual table (creation + INSERT/UPDATE/DELETE triggers in MIGRATION_11_12); `NoteSearchScreen` debounces typing at 150 ms, prefix-matches via `NoteSearchRepository`, and renders highlighted snippets via FTS4's `snippet()`. Audio-synced ink: `note_audio` table (MIGRATION_12_13), `AudioRecorder` / `AudioPlayer`, `RECORD_AUDIO` requested at first tap, the `AudioRecordingBar` / `AudioPlaybackBar` Compose surfaces, `StrokeCodec` v1 + v2 (one-byte version header, fifth `t` lane), `StrokeReplayer` for clip-against-position, and `DrawingSurface.recordingStartedAt` so live strokes encode v2 only while a recording is active. **Outstanding on this branch:** wiring `StrokeReplayer` into the actual rendering pipeline so playback animates the strokes (the data model + replay maths are landed; the surface still draws all items regardless of the player position) and 9.5 on-device verification. 8.5 + the pending Phase 5.5 / Phase 6 / Phase 7 device passes are also still outstanding.
- **Next sub-phase:** 9.5 on-device verification matrix (after wiring `StrokeReplayer` into `DrawingSurface`'s scene-bitmap pass for the playback animation).
- **Last verified device pass:** n/a (5.5, Phase 6 matrix, Phase 7 matrix, Phase 8 matrix, and Phase 9 matrix pending)
...
- [x] **9.1** Notebook entity + Notebooks list screen (separate from Notes list) ([details](./ARTIST_CANVAS_PHASE_9.md#sub-phase-91--notebook-entity--list-screen)) — `Notebook` + `notebooks` table (MIGRATION_11_12), `Note.notebookId` FK, `NotebookDao` / `NotebookRepository` (FK-cascading delete with file-on-disk sweeps for thumbnails / images / audio), `NotebooksListScreen` 2-col cover grid + long-press rename / delete, `NewNotebookSheet` picks A4 / Letter / half-letter landscape × plain / dot / line / graph × eight cover colours, "Notebooks" bottom-nav tab. `NotesListScreen` filters to `notebookId IS NULL` so notebook pages no longer pollute the notes list.
- [x] **9.2** Paginated page view (fixed-size pages stacked vertically; reuses `DrawingSurface`) ([details](./ARTIST_CANVAS_PHASE_9.md#sub-phase-92--paginated-page-view)) — implemented as a navigator-rail variant rather than the multi-surface `LazyColumn` of `DrawingSurface` instances the spec sketched. `PageThumbnailRail` renders one cell per page (thumbnails via the Phase 8.2 `FrameThumbnailRenderer`), tap flies the existing single `DrawingSurface` viewport to that page, "Add page" appends a uniform-sized frame below the last one and updates the notebook's `updatedAt`. The page-flip animation, two-page spread, and parallel surface instances stay deferred per the phase-doc fallback notes (the same "engine doesn't change" contract holds — pages are world-space frames the viewport snaps between).
- [x] **9.3** Cross-note / cross-notebook OCR search ([details](./ARTIST_CANVAS_PHASE_9.md#sub-phase-93--cross-notebook-ocr-search)) — standalone `notes_ocr_fts` virtual FTS4 table (no `content=` link; manual `notes_fts_ai` / `_au` / `_ad` triggers in MIGRATION_11_12 so the raw `NoteDao.updateOcrText` UPDATE path keeps the index in sync), `NoteSearchDao` with FTS4 `snippet('<b>', '</b>', '…', -1, 16)`, `NoteSearchRepository` (alphanumeric tokenisation + `term*` prefix matching), `NoteSearchScreen` 150 ms-debounced typing, `<b>`-aware AnnotatedString highlighting, results joined with `notebooks` to surface notebook title + page label. Search button in the Notes + Notebooks list TopAppBars. Per-result page-ordinal resolution is the first frame today; per-snippet ocr → page mapping is deferred (notebooks open at page 1 and the user scrolls).
- [x] **9.4** Audio recording with synced ink playback (per-sample timestamps, stroke format bump) ([details](./ARTIST_CANVAS_PHASE_9.md#sub-phase-94--audio-synced-ink)) — `note_audio` table (MIGRATION_12_13, FK-cascading delete + on-disk file cleanup), `AudioRecorder` (AAC / MP4 @ 64 kbps under `filesDir/note-audio/`, orphan sweep at app start), `AudioPlayer` (`MediaPlayer` wrapper, 30 Hz `positionMs` `StateFlow`), `AudioRecordingBar` requests `RECORD_AUDIO` at first tap and shows elapsed-time chip, `AudioPlaybackBar` renders per-clip chips + a scrubber that drives `seekTo`. `StrokeCodec` v1 + v2: v2 prepends a `0x02` version byte and stores `(x, y, p, tilt, t)` per sample; `decode` round-trips v2 → v1-shape data so every existing call site keeps working unmodified; v1 strokes synthesize `t = 0` via `decodeWithT` so they're transparently skipped by the replayer. `DrawingSurface.recordingStartedAt` toggles the live-stroke encoder between v1 and v2 — strokes drawn outside a recording stay binary-identical to pre-9.4 commits. `StrokeReplayer` provides `clipStroke(item, currentPositionMs)` + `itemsForRecording(items, durationMs)` for the playback render pass; **wiring those into the surface scene bitmap is the remaining piece** — today playback advances the audio and the scrubber but strokes are drawn statically rather than progressively.
